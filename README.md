...
but it turns out that controlling them within a VM setup is subpar: the
motherboard's RGB controller shows up as a USB device which you can pass through
to a guest and control via OpenRGB, but it's not really worth the hassle. I
picked up a [standalone RGB
...
controller out the bottom of the case.

# BIOS

As this build is subject to Intel's recent [burn-in
...
It's currently running the following BIOS settings (notes are somehwat terse but
...
* Four cycles of memtest86 (the version on Ubuntu's install image) which passed 100%
* Installing a temporary Ubuntu install that I didn't feel bad about trashing
* A few hours of s-tui while ensuring temperatures stayed below 85C or so
  (details in 'Cooling Calibration' below)
* A handful of runs on Geekbench on the Linux host to establish baseline Linux
  benchmarks
* A scratch bare-metal Windows 11 install to establish baseline Windows benchmarks
...
565.57.01) makes a pretty big difference in idle power consumption (this
is noted in Nvidia's release notes). If you want to install the open drivers, it
suffices to add `nvidia-kernel-open-dkms` to the `apt install
nvidia-drivers-cuda libnvidia-ml1` command below; the package dependencies are
smart enough to notice that you're bringing in the open driver source.

RTD3 support on desktop is still out of reach on this setup; I *think* it's
because this motherboard doesn't expose the relevant ACPI hooks, but adding in
debug info to the build module is a bit of a pain in the ass given the way that
the kernel source packages come with a pre-compiled nv-kernel.so (which is where
the [relevant places to add logging to
see](https://github.com/NVIDIA/open-gpu-kernel-modules/blob/main/src/nvidia/arch/nvalloc/unix/src/dynamic-power.c)
are defined. The end goal here is to have the card go into RTD3 when not being
passed through, but this is a bit more tinkering than I'm able to throw at this
for the time being.
...
* Make the following the only active `option` lines in `/etc/modprobe.d/nvidia.conf`
    ```
...
* Add the following to `/usr/local/bin/powersave` and set 755 (we'll use
  this in our VM hook scripts later)
    ...
    # Exit if we're running the VM
    /usr/sbin/qm status 101 | grep -q running && exit 0
...
The following two changes allow the Windows VM to be started by typing `run` on
the keyboard. `cec-utils` will be used in the Windows VM hook scripts to
automatically turn the TV on and off when the VM starts or stops.
...
* Add the following to `/usr/local/bin/run` and set 755
    ```
    sudo /usr/sbin/qm start 101
    ```
* Add the following to `/usr/local/bin/stop` and set 755
    ```
    sudo /usr/sbin/qm stop 101
    ```
...
* `apt remove nvidia-driver-cuda libnvidia-ml1`
* `apt purge *nvidia*`
* `apt autoremove`
* Run the previous two commands until there are no nvidias left in `dpkg -l`
* `apt clean`
