...
* Installing a temporary Ubuntu install that I didn't feel bad about treating
  badly
...
* I used [s-tui](https://github.com/amanusk/s-tui) to fully load all cores.
  ...
  load, while still having all cores running at their max of 5.1GHz / 3.8GHz. IF
...
* Under full load, I tinkered with the fan curves in the BIOS to allow the fans
  ...
  BIOS section above reflect this
...
brining the machine from a ~45W idle to a ~25W idle

* Add the following to `/etc/apt/sources.d/non-free.list`
    ```
    deb http://ftp.ca.debian.org/debian bookworm non-free non-free-firmware
    deb http://ftp.ca.debian.org/debian bookworm-updates non-free non-free-firmware
    ```
* `apt update`
* `apt install pve-headers`
* `apt install libnvidia-cfg1 nvidia-kernel-source nvidia-kernel-common nvidia-driver`
* `systemctl enable nvidia-persistenced`
* Add the following to `/usrl/local/bin/gpu_passthrough` and set 755 (we'll use
  this in our VM hook scripts later)
    ...
    ```
* Add the following to `/usr/local/bin/gpu_powersave` and set 755 (we'll use
  this in our VM hook scripts later)
    ```
    echo 0000:01:00.0 > /sys/bus/pci/devices/0000:01:00.0/driver/unbind
    echo 0000:01:00.1 > /sys/bus/pci/devices/0000:01:00.1/driver/unbind
    ...
    ```
* Add the following to `/usr/local/bin/gpu_powersave_safe` and set 755
    ```
    # Quit if the nvidia driver already loaded
    readlink /sys/bus/pci/devices/0000:01:00.0/driver | grep -q nvidia && exit 0

    # Quit if our Windows VM is running (replace `101` with the ID of your Windows VM)
    /usr/sbin/qm status 101 | grep -q running && exit 0
    /usr/local/bin/gpu_powersave
    ```
* Run `crontab -e` as root and add `*/2 * * * * /usr/local/bin/gpu_powersave_safe`

### Misc power savings

This section is a work in progress

* Add `blacklist btusb` to `/etc/modprobe.d/blacklist.conf`
...
* Add the following to `/var/lib/vz/snippets/gpu.sh` and set 755
    ```
#!/usr/bin/env bash
    ...
    exit 0
    ```
* Edit the `/etc/pve/qemu-server/1??.conf` for the above VM and add a line like `hookscript: local:snippets/gpu.sh`
